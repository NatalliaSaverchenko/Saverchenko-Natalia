...
function getLinearEquationRoot(a, b) {
  ...
  if (a === 0 && b === 0) {
    return 'root is any number'
  }
  if (a === 0 && b != 0) {
    return 'root is not existed'
  }
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let a_mod = Math.sqrt(x1 ** 2 + y1 ** 2)
  let b_mod = Math.sqrt(x2 ** 2 + y2 ** 2)
  let a_b_multiply = x1 * x2 + y1 * y2
  return Math.acos(a_b_multiply / (a_mod * b_mod))
}
...
function roundToPowerOfTen(num, pow) {
  let koef = 10 ** pow
  let new_num = num / koef

  if (pow === 0) {
    return num
  } else {
    return Math.round(new_num) * koef
  }
}
